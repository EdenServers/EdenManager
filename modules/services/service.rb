module Monitoring
  class Service
    attr_accessor :name, :working_dir, :pid_file, :daemon_id, :alive, :monitoring
    ...
    #Start the program
    def start_service
      unless service = is_service_installed?
        @daemon_id = get_pid(pid_file)
        if !process_alive?
          @daemon_id = System.daemonize(service[:startCommand], {working_dir: service[:folderNamec], pid_file: pid_file})
